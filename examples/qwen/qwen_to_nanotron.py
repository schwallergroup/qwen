...
def convert_hf_to_nt(model_hf: AutoModelForCausalLM, model_nt: LlamaForTraining, config: NanotronLlamaConfig):
    ...
    for module_name_nt, module_nt in model_nt.named_modules():
        for param_name_nt, param_nt in module_nt.named_parameters(recurse=False):
            ...
            # to q, k, v.
            #print("module list")
            #print(f"{module_name_nt}.{param_name_nt}")
            if "qkv_proj" in module_name_nt and "bias"==param_name_nt:
                #print("bias")
                #print(module_name_nt)
            ...
            elif "qkv_proj" in module_name_nt and "weight"==param_name_nt:
                #print("weight")
                #print(module_name_nt)
                key_k, key_q, key_v = sorted(nt_to_hf[f"{module_name_nt}.{param_name_nt}"])
                q = hf_sd[key_q]
            ...
            elif "qkv_proj" not in module_name_nt:
...
def convert_checkpoint_and_save(checkpoint_path: Path, save_path: Path):
    ...
    nanotron_model = load_nanotron_model(model_config=model_config, hf_weights=hf_model)
    print("state")
    print(nanotron_model.state_dict())
